...
export class c_planEstudio {
    ...
    configurarEventos() {
        ...
        $('.nuevo').off('click').on('click', () => {
            this.modoEdicion = false;
            this.planActual = null;
            u_planEstudio.limpiarFormulario(); // Esto ahora auto-completará el periodo
        ...
        $(document).off('click', '.editar-creditos').on('click', '.editar-creditos', (e) => {
            e.stopPropagation();
            const semestreId = $(e.currentTarget).data('semestre');
            const index = $(e.currentTarget).data('index');
            this.editarCreditosAsignatura(semestreId, index);
        });

    }

    // metodo de prueba
    editarCreditosAsignatura(semestreId, index) {
        const asignatura = u_planEstudio.asignaturasPorSemestre[semestreId]?.[index];
        if (!asignatura) return;
        
        // Usar la misma función de solicitar créditos pero para edición
        u_planEstudio.solicitarCreditos(
            {
    ...
    /*async cambiarEstadoPlan(id) {
        ...
        const nuevoVigente = plan.vigente === 'Sí' ? 'Sí' : 'No';
