...
export class c_matricula {
    ...
    async guardarMatricula() {
        if (!u_matricula.validarSeccion1() || !u_matricula.validarSeccion2($('#esBecario').is(':checked')) || 
        ...
        try {
            ...
            if ($('#nombreOCorreoUsuario').val()) {
                idUsuario = await this.crearUsuario(); console.log(idUsuario)
                if (!idUsuario) return;
            }

            const idEstudiante = await this.crearEstudiante(idUsuario); console.log(idEstudiante.id)
            if (!idEstudiante) return;

            await this.crearFamiliares(idEstudiante.id);

            const idMatricula = await this.crearMatricula(idEstudiante.id); console.log(idMatricula.id)
            if (!idMatricula) return;

            if ($('#esBecario').is(':checked')) {
                await this.crearBecas(idEstudiante.id);
            }

            await this.crearPago(idMatricula.id);
    ...
    async crearBecas(idEstudiante) {
        // guardamos primero las becas
        const becas = new FormData();
        becas.append('institucionBeca', $('#comboNombreInstitucionBeca').val());
        becas.append('tipoBeca', $('#tiposBeca').val());

        const idBeca = await m_beca.insertarBeca(becas); console.log(idBeca)

        const principal = new FormData();
        principal.append('idEstudiante', idEstudiante);
        principal.append('idBeca', idBeca);
        ...
        await m_estudianteBeca.insertarEstudianteBecado(principal);
